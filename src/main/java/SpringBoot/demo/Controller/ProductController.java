...
public class ProductController {
    ...
    // GET /products - Lấy tất cả sản phẩm (công khai)
    @Operation(summary = "Lấy danh sách sản phẩm", description = "Lấy tất cả sản phẩm (không cần đăng nhập)")
    @ApiResponses(value = {
    ...
    public ResponseEntity<ApiResponse<java.util.List<Product>>> getAllProducts() {
        ...
        try {
            ...
            ApiResponse<java.util.List<Product>> response = ApiResponse.success(products);
            return ResponseEntity.ok(response);
            
        } catch (Exception e) {
            ApiResponse<java.util.List<Product>> errorResponse = ApiResponse.error("Lỗi hệ thống: " + e.getMessage());
