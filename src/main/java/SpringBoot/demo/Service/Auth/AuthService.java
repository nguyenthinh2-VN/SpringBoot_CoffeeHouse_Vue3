...
public class AuthService implements UserDetailsService {
    ...
    public AuthResponse googleLogin(GoogleTokenRequest request) {
        try {
            ...
            GoogleIdToken idToken = googleTokenVerifier.verifyToken(request.getToken());
            
            if (idToken == null) {
                return AuthResponse.error("Google token không hợp lệ");
            }

            // Lấy email từ token
            String email = googleTokenVerifier.getEmailFromToken(idToken);
            
            if (email == null || email.isEmpty()) {
                return AuthResponse.error("Không thể lấy email từ Google token");
            }
            ...
            if (existingUser.isPresent()) {
                // User đã tồn tại, chỉ lấy thông tin
                ...
                user = new User();
                user.setEmail(email);
                user.setUsername(null); // Bỏ trống username
                user.setPassword(null); // Bỏ trống password
                user.setRole(Role.USER);
                user.setIsActive(true);

                // Lấy thêm thông tin từ Google nếu có
                String name = googleTokenVerifier.getNameFromToken(idToken);
            ...
            return AuthResponse.success("Đăng nhập bằng Google thành công", authData);
